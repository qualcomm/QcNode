...
- [2. QCNode GL2DFlex Data Structures](#2-qcnode-gl2dflex-data-structures)
  - [2.1 The details of GL2DFlex\_Config\_t](#21-the-details-of-gl2dflex_config_t)
  - [2.2 The details of GL2DFlex\_InputConfig\_t](#22-the-details-of-gl2dflex_inputconfig_t)
  - [2.3 The details of GL2DFlex\_ROIConfig\_t](#23-the-details-of-gl2dflex_roiconfig_t)
  - [2.4 The details of GL2DFlex\_ImageResolution\_t](#24-the-details-of-gl2dflex_imageresolution_t)
- [3. QCNode GL2DFlex APIs](#3-qcnode-gl2dflex-apis)
- [4. Typical Use Case](#4-typical-use-case)

# 1. Introduction
The QCNode GL2DFlex sample offers a range of camera image processing functions, such as cropping, downscaling, upscaling, and color conversion. It works based on OpenGL ES and is compatible with the HGY platform. Additionally, it provides a user-friendly API and a high-performance function library.

# 2. QCNode GL2DFlex Data Structures

- [GL2DFlex_Config_t](GL2DFlex.hpp#L90)
- [GL2DFlex_InputConfig_t](GL2DFlex.hpp#L81)
- [GL2DFlex_ROIConfig_t](GL2DFlex.hpp#L72)
- [GL2DFlex_ImageResolution_t](GL2DFlex.hpp#L64)
...
# 3. QCNode GL2DFlex APIs
- [Initialize the GL2DFlex component](GL2DFlex.hpp#L116)
- [Start the GL2DFlex pipeline](GL2DFlex.hpp#L123)
- [Stop the GL2DFlex pipeline](GL2DFlex.hpp#L130)
- [Deinitialize the GL2DFlex component](GL2DFlex.hpp#L137)
- [Execute the GL2DFlex pipeline](GL2DFlex.hpp#L147)
- [Register shared buffers for each input](GL2DFlex.hpp#L160)
- [Register shared buffers for output](GL2DFlex.hpp#L73)
- [Deregister shared buffers for each input](GL2DFlex.hpp#L183)
- [Deregister shared buffers for output](GL2DFlex.hpp#L193)

# 4. Typical Use Case
The QCNode GL2DFlex component is used for image cropping, color conversion and downscaling/upscaling. User just needs to set input/output configuration parameters, then call the QCNode Init/Start API to launch this component. Here is an example use case that converts UYVY to NV12 and do cropping:
...
```c++
    ...
    QCNodeStatus_e ret = QCNode_STATUS_OK;

    GL2DFlexConfig.numOfInputs = 1;
    for ( size_t i = 0; i < GL2DFlexConfig.numOfInputs; i++ )
    {
        GL2DFlexConfig.inputConfigs[i].inputFormat = QCNode_IMAGE_FORMAT_UYVY;
    ...
    QCNodeImageFormat_e outputFormat = QCNode_IMAGE_FORMAT_NV12;
...
```c++
    ...
    {
        ret = bufMgr.Allocate(
                    ImageBasicProps_t( GL2DFlexConfig.inputConfigs[i].inputResolution.width,
    ...
    ret = bufMgr.Allocate(
                ImageBasicProps_t( outputWidth, outputHeight, GL2DFlexConfig.outputFormat ),
                output );
    for ( size_t i = 0; i < numInputs; i++ )
    {
...
Reference: 
- [gtest_GL2DFlex](./gtest_GL2DFlex.cpp)
- [SampleGL2DFlex](../../source/SampleGL2DFlex.cpp)
