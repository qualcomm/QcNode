...
#ifndef RIDEHAL_SAMPLE_SHARED_RING_HPP
#define RIDEHAL_SAMPLE_SHARED_RING_HPP
...
typedef struct SharedRing_DataFrame
{
    ...
    uint8_t reserved[1024 - ( sizeof( RideHal_SharedBuffer_t ) + sizeof( uint64_t ) * 2 +
...
typedef struct SharedRing_Ring
{
    ...
    uint8_t reserved0[4096 -
                      ( sizeof( RideHal_SpinLock_t ) + sizeof( int32_t ) * 2 + sizeof( uint32_t ) +
                        sizeof( uint16_t ) * ( SHARED_RING_NUM_DESC + 2 ) + SHARED_RING_NAME_MAX )];

public:
    RideHalError_e Push( uint16_t idx );
    RideHalError_e Pop( uint16_t &idx );
    uint32_t Size();
    void SetName( std::string name );
} SharedRing_Ring_t;
...
#endif   // RIDEHAL_SAMPLE_SHARED_RING_HPP
