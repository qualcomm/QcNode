...
TEST( VideoEncoder, SANITY_VideoEncoder_ConfigBuffer )
{
    ...
    RideHal_SharedBuffer_t *inBufferList = new RideHal_SharedBuffer_t[config.numInputBufferReq];
...
TEST( VideoEncoder, SANITY_VideoEncoder_Resolution )
{
    ...
    config.height = 2160;
    config.bitRate = 20000000;
    config.inFormat = RIDEHAL_IMAGE_FORMAT_P010;
...
TEST( VideoEncoder, SANITY_VideoEncoder_InitError )
{
    ...
    for ( i = 0; i < 8; i++ )
    {
        ret = inBufferList[i].Allocate( 176, 144, RIDEHAL_IMAGE_FORMAT_NV12 );
        ASSERT_EQ( RIDEHAL_ERROR_NONE, ret );
        RideHal_ImageProps_t imgProps;
        imgProps.batchSize = 1;
        imgProps.width = 176;
...
TEST( VideoEncoder, SANITY_VideoEncoder_OtherError )
{
    ...
    ASSERT_EQ( RIDEHAL_COMPONENT_STATE_INITIAL, veTest1.GetState() );
