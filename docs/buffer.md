*Menu*:
- [1. QC Buffer Data Structures](#1-qc-buffer-data-structures)
  - [1.1 The details of image properties.](#11-the-details-of-image-properties)
  - [1.2 The details of QCSharedBuffer_t.](#12-the-details-of-qcnode_sharedbuffer_t)
- [2. QC buffer APIs](#2-qc-buffer-apis)
- [3. QCSharedBuffer_t Examples](#3-qcnode_sharedbuffer_t-examples)
  - [3.1 A QCSharedBuffer_t image for BEV kind of AI model](#31-a-qcnode_sharedbuffer_t-image-for-bev-kind-of-ai-model)
...
And in fact, the QCSharedBuffer_t APIs are based on the platform DMA related APIs (PMEM for QNX, dma-buf for Linux). 
  - For QNX, check [QCDmaAllocate](../source/common/Buffer_QNX.cpp#L30) and [QCDmaFree](../source/common/Buffer_QNX.cpp#L82).
  - For Linux, check [QCDmaAllocate](../source/common/Buffer_Linux.cpp#L62) and [QCDmaFree](../source/common/Buffer_Linux.cpp#L138).
...
shareBuffer.buffer.usage = QC_BUFFER_USAGE_DEFAULT;
shareBuffer.buffer.flags = 0;
shareBuffer.size = size;
shareBuffer.offset = 0;
shareBuffer.type = QC_BUFFER_TYPE_IMAGE;
...
// and then this can be feed into a QC Component

remap.Execute(&shareBuffer, 1, ...);
```

And another thing, the QCSharedBuffer_t can be shared between components, but it has no life cycle management ability. Here, the QC Sample Application has a demo that using C++ std::shared_ptr to demonstrate that how to do the buffer life cycle management between the components that running in the same process but in different threads, refer [The QC Sample Buffer Life Cycle Management](./sample-buffer-life-cycle-management.md).
...
## 3.1 A QCSharedBuffer_t image for BEV kind of AI model
...
public:
    QCStatus_e Init( void )
    {
        ...
        {
            ret = m_buffers.Allocate( 3840, 2160, QC_IMAGE_FORMAT_NV12 );
        }

        return ret;
    }

    QCStatus_e Run( QCSharedBuffer_t *pInput )
    {
        QCSharedBuffer_t *pBuffer = m_buffers[m_index];
...
private:
    QCSharedBuffer_t m_buffers[4];   // A case that want 4 ping-pong buffers.
    uint32_t m_index = 0;
}
```
